    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(data, function(index, customer) {
                ...
                row.append('<td class="editable">' + customer.customerName + '</td>');
                row.append('<td class="editable">' + customer.customerPhone + '</td>');
            ...
            table.on('click', '.modifyButton', function () {
                ...
                if (!isEditing) {
                    ...
                    $.ajax({
                        url: '/updateCustomer', // Django 後端對應的 URL
                        type: 'POST',           // POST 請求
                        data: {
                            id: row.find('td:first').text(),             // 取得第一欄的 ID
                            customerName: row.find('td:nth-child(2)').text(), // 第二欄的供應商名稱
                            customerPhone: row.find('td:nth-child(3)').text() // 第三欄的供應商電話
                        },
            ...
            table.on('click', '.deleteButton', function () {
                const confirmed = window.confirm("是否確認要刪除客戶？");
                if (confirmed){
                    let row = $(this).closest('tr');
                    $.ajax({
                        url: '/deleteCustomer', // Django 後端 API 路徑
